# perplexity_agent_skills.md [DISTILLED]

**Summary:** This document provides Perplexity's internal guide on designing, refining, and maintaining agent skills. Key insights include the concept of the "skill tax" (index budget), the necessity of the "gotchas flywheel" for append-only failure tracking, the strictly 3-tiered context architecture in Perplexity (Index, Load, Runtime), the use of `config.json` for first-run setup, and the emphasis on writing evaluations *before* the skill (incorporating both positive and negative examples across multiple model orchestrators). [See: ## WHAT IS A SKILL?], [See: ## HOW TO BUILD A SKILL].
...
| **references/** | Heavy docs loaded only when a condition is met. e.g., "Read api-errors.md if API returns non-200" |
| **assets/** | Output templates the agent copies and fills. e.g., report-template.md, output schemas |
...
Use all the available tools every time you’re writing and maintaining a Skill. If you want to learn more, the Agent Skills website has plenty of good examples, and both our internal Github repo and the public ecosystem contain many examples of well-designed Skills.