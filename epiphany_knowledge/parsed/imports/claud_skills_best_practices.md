# claud_skills_best_practices.md [DISTILLED]

**Summary:** This document provides a comprehensive guide on authoring effective Agent Skills. Key extractions include:
- **Core Principles:** Conciseness, matching degrees of freedom to the task, and testing across different models.
- **Skill Structure:** YAML frontmatter constraints (gerund naming convention), writing clear descriptions for discovery, and using progressive disclosure (moving details to separate files, one-level deep).
- **Workflows:** Using checklists for complex tasks and implementing plan-validate-execute loops.
- **Code & Scripts:** Handling errors explicitly, using verifiable intermediate outputs, and relying on the filesystem to manage context efficiently.
- **Evaluation:** Building evaluations first to measure effectiveness and iterating on skills by observing agent navigation.
[See: ## Core principles], [See: ## Skill structure], [See: ## Progressive disclosure patterns], [See: ## Evaluation and iteration].

**Source:** /home/qt/dev/epiphany/docs/skill_references/claud_skills_best_practices.md
