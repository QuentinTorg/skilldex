# claude_skills_overview.md [DISTILLED]

**Summary:** This document provides an overview of Agent Skills architecture, primarily focusing on progressive disclosure (metadata, instructions, code), security risks (tool misuse, untrusted sources), and environmental constraints (network, packages, cross-surface sharing) [See: ## How Skills work, ## Security considerations, ## Limitations and constraints]. Extracts updated `skill-specification.md` and created `skill-security.md` and `skill-environments.md`.

**Source:** /home/qt/dev/epiphany/docs/skill_references/claude_skills_overview.md
