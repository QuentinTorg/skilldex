...
- **Execution Model / Lifecycle:** Is it a long-running daemon, a cron job, a stateless web server, an event-driven worker? *(Tip: Use a Mermaid state diagram for complex lifecycles).*
...
- **Error Handling:** What happens when things fail (e.g., standard HTTP codes, timeout behaviors).ct protobuf/JSON schema.
